...
public class Ai_8 : MonoBehaviour

{
    public static bool bang_8 = false;
    float stoptime = 0;
    ...
    private void Update()
    {
        ...
        if (GameObject.Find("shower8.0").GetComponent<FieldOfView>().canSeePlayer == true)//can it see player
        {
    ...
    void Attack()
    {
        if (Vector3.Distance(Target.transform.position, transform.position) <= AttackDistance && AnimationConroller.GetFloat("speed") == 0 && !bang_8)
