...
{
    public static bool bang_4 = false;
    float stoptime = 0;
    ...
    private void Update()
    {
        ...
        if (GameObject.Find("shower4.0").GetComponent<FieldOfView>().canSeePlayer == true)//can it see player
        {
    ...
    void Attack()
    {
        if (Vector3.Distance(Target.transform.position, transform.position) <= AttackDistance && AnimationConroller.GetFloat("speed") == 0 && !bang_4)
